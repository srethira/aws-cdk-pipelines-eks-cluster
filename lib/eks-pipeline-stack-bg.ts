...
export class EksPipelineStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    // const bucket = s3.Bucket.fromBucketName(this, 'Bucket', 'medtronic-cdk-eks-infra-app');
    
    const sourceBucket = new s3.Bucket(this, 'medtronic-cdk-eks-infra-app', {
      versioned: true, // a Bucket used as a source in CodePipeline must be versioned
    });
    
    const pipeline = new CodePipeline(this, 'Pipeline', {
      synth: new ShellStep('Synth', {
        input: CodePipelineSource.s3(sourceBucket, 'aws-cdk-pipelines-eks-cluster-main.zip'),
        commands: [
